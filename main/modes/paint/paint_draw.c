...
const paintHelpStep_t helpSteps[] =
{
    ...
    { .trigger = { .type = RELEASE, .data = TOUCH_Y, }, .prompt = "Now, let's change the brush size. Just tap X on the TOUCH PAD to increase the brush size by 1" },
    { .trigger = { .type = RELEASE, .data = BTN_A, }, .prompt = "Press A to draw again with the larger brush!" },
    { .trigger = { .type = RELEASE, .data = TOUCH_X, }, .prompt = "Wow! Now, to decrease the brush size, just tap Y on the TOUCH PAD!" },
