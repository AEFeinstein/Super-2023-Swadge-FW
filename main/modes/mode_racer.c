...
void rChangeState(racer_t newState);
...
void ttScoresInput(void)
{
    ...
    if(tiltrads->holdingClearScore && ttIsButtonDown(BTN_SCORES_CLEAR_SCORES))
    {
        ...
        if (tiltrads->clearScoreTimer >= CLEAR_SCORES_HOLD_TIME)
        {
            ...
            memset(tiltrads->highScores, 0, NUM_R_HIGH_SCORES * sizeof(uint32_t));
            saveHighScores();
            loadHighScores();
        }
    }
...
void ttGameDisplay(void)
{
    ...
    // ROTATE
    int16_t rightControlTextX = tiltrads->disp->w - textWidth(&(tiltrads->ibm_vga8), "ROTATE") - 32;
...
void ttScoresDisplay(void)
{
    ...
    // XX. 99999
    for (int i = 0; i < NUM_R_HIGH_SCORES; i++)
    {
        int16_t x0 = 0;
                ...
                uint32_t gridData[][gridCols])
{
    ...
    if ((targetPos.c == tetrad->topLeft.c + 1 ||
            ...
            abs(tiltrads->ttAccel.y - tiltrads->ttLastTestAccel.y) <= ACCEL_JIRER_GUARD)
    {
